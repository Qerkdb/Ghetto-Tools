script_name("Ghetto Tools by Squzi")
script_version("0.1")
script_author('Squzi')
...
function imgui.OnDrawFrame()
    --if window.v then
        ...
        imgui.Begin('Ghetto Tools by Squzi', window, imgui.WindowFlags.NoTitleBar, imgui.WindowFlags.AlwaysAutoResize)
        ...
        if menu == 1 then
            ...
            imgui.Text(u8'Автор доработки Squzi')
            ...
            imgui.TextColoredRGB('Спасибо за пользование {525497}Ghetto Tools{FFFFFF})')
        ...
        if menu == 2 then
            if imgui.Checkbox('##Sbiv', checksbiv) then
                ...
                inicfg.save(cfg,'Ghetto Tools/Ghetto Tools.ini')
            end
            imgui.SameLine()
        ...
        if imgui.Button(u8'Сбросить настройки', imgui.ImVec2(-1, 25)) then
            ...
            os.remove(getWorkingDirectory()..'/config/Ghetto Tools/Ghetto Tools.ini')
...
function autoupdate(json_url, prefix, url)
    lua_thread.create(function()
        ...
        while update ~= false do wait(100) end
                                                                                                                                                                                                                                                                                                                                                                                                                           ...
                                                                                                                                                                                                                                                                                                                                                                                                                           function LoadScript() if thisScript().filename ~= 'Ghetto Tools by Squzi.lua' then msg('Название скрипта было изменено, скрипт отключён') msg('Измените название скрипта на "Ghetto Tools by Squzi.lua"') os.rename(getWorkingDirectory()..'\\'..thisScript().filename,getWorkingDirectory()..'\\Ghetto Tools by Squzi.lua') end end

function main()
    while not isSampAvailable() do wait(200) end
   -- while not sampIsLocalPlayerSpawned() do wait(200) end
    if not doesDirectoryExist('moonloader/config/Ghetto Tools') then createDirectory('moonloader/config/Ghetto Tools') end
    if not doesFileExist(getWorkingDirectory()..'/config/Ghetto Tools/Ghetto Tools.ini') then inicfg.save(cfg, 'Ghetto Tools/Ghetto Tools.ini') end
    if not doesFileExist(getWorkingDirectory()..'/config/Ghetto Tools/bell.wav') then
        downloadUrlToFile('https://github.com/Venibon/Ghetto-Helper/raw/main/bell.wav', getWorkingDirectory()..'/config/Ghetto Tools/bell.wav')
    end
...
function sampev.onSendGiveDamage(playerId,damage)
	if cfg.config.Bell then
		local audio = loadAudioStream('moonloader/config/Ghetto Tools/bell.wav')
...
function msg(arg)
    sampAddChatMessage('{FFFFFF}[{5d2d89}Ghetto Tools{FFFFFF}]{5d2d89}: {FFFFFF}'..arg, -1)
end
